    ...
    def get_ilra_dims(self, config):
        ...
        for ly in range(self.config.num_hidden_layers):
            for module in self.config.modules_to_apply:
                ...
                # breakpoint()
                for dim in dims:
                    divisible_dims.append(self.check_ilra_dim(dim, self.config.hidden_size, self.config.intermediate_size))
                ilra_dims[ly][module] = dims
        return ilra_dims
...
class PilraLayer():
    def __init__(
        ...
        # Optional dropout
        if pilra_dropout > 0.:
            self.krona_dropout = nn.Dropout(p=pilra_dropout)
        else:
...
class Linear(nn.Linear, PilraLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        d_c, d_eid, d_m = pilra_dims
        
        self.d_c_A = nn.Parameter(self.weight.new_zeros((d_c, out_features//d_c)))
        self.krona_B = nn.Parameter(self.weight.new_zeros((in_features//pilra_dim, pilra_dim)))
        self.scaling = self.pilra_alpha / self.pilra_dim
        
        for pilra_dim in pilra_dims:
            if pilra_dim > 0:
                self.krona_A = nn.Parameter(self.weight.new_zeros((pilra_dim, out_features//pilra_dim)))
                self.krona_B = nn.Parameter(self.weight.new_zeros((in_features//pilra_dim, pilra_dim)))
                self.scaling = self.pilra_alpha / self.pilra_dim
